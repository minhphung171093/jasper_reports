...
class Report:
    ...
    def execute(self):
        ...
        """
        logger = logging.getLogger(__name__)
        if self.name == 'report.jasper_production_month_report_email':
            self.outputFormat = 'xls'
        elif self.name == 'report.jasper_daily_product_production_email':
            self.outputFormat = 'html'
        elif self.name == 'report.jasper_monthly_grade_email_report':
            self.outputFormat = 'html'
        elif self.name == 'report.jasper_production_pcp_report_email':
            self.outputFormat = 'xls'
            
        elif self.name == 'report.jasper_tgi_production_report_email':
            self.outputFormat = 'xls'            
        elif self.name == 'report.jasper_tgi_email_report':
            self.outputFormat = 'html'
        
        elif self.name == 'report.jasper_plant_wise_motor_damage_report':
            self.outputFormat = 'html'
        
        #elif self.name == 'report.jasper_plant_wise_motor_damage_email_report':
            #self.outputFormat = 'html'
            
        elif self.name == 'report.jasper_zircon_quality_report':
            self.outputFormat = 'xls'
        elif self.name == 'report.jasper_rutile_quality_report':
            self.outputFormat = 'xls'
        elif self.name == 'report.jasper_email_manage_chemical_details':
            self.outputFormat = 'xls'
        else:
        ...
        logger.info("Requested report: '%s'" % self.reportPath)
        self.report = JasperReport( self.reportPath )
        #raise osv.except_osv(_('Warning!%s'),_(self.reportPath)) 
        # Create temporary input (XML) and output (PDF) files
        fd, dataFile = tempfile.mkstemp()
        os.close(fd)
        fd, outputFile = tempfile.mkstemp()
        ...
        if self.report.language() == 'xpath':
            if self.data.get('data_source','model') == 'records':
                generator = CsvRecordDataGenerator(self.report, self.data['records'])
################################################################################
#FIX ADDED BY Sridhar ON 07 sep 2014 for adding sub reports
################################################################################
...
################################################################################
    ...
    def executeReport(self, dataFile, outputFile, subreportDataFiles):
        ...
        # print kkkkkkkkkk
#############################################################
#Fix Added by Sridhar on 07 sep 2014
#############################################################
